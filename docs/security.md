...
This is the part that decides whether the package fits a deployment, so it is worth being blunt about.

Each person needs two things arranged, and only one of them belongs to whoever runs the server. At the provider, their WebID has to be in a claim, which is administrator work in the provider's own console, once per person. In their profile, the grant above has to exist, which is RDF that someone has to write into a document on the pod.

Nothing in this package writes it. There is no page in the server for it, no command-line tool here, and no import format. For a person who is comfortable editing their own profile document, it is a copy-paste. For everyone else, it is a wall, and a support request that lands on whoever runs the server.
