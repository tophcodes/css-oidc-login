...
**`scopes`** defaults to `openid profile`. Keep `profile` unless the provider is known not to need it — see the [Pocket ID notes](./providers.md#pocket-id-webid-in-a-custom-claim), where dropping it silently costs the WebID claim.
...
The build compiles `src/` with `tsc` and then generates the component descriptions into `dist/components/`. The `@context` URL in the configuration must match the one that generation stamps into `dist/components/context.jsonld`; it tracks the package's major version, which is why a 0.x release is `^0.0.0`.
